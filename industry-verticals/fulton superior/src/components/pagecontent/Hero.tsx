...
/* SuperiorCourt — welcome title, serif prompt, search + Search CTA, popular links */
export const SuperiorCourt = (props: HeroProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  const [isSearchOpen, setIsSearchOpen] = useState(true);
  const searchPlaceholder = 'Search the court website...';

  return (
    <div
      className={`component hero hero-superior-court ${sxaStyles}`}
      id={id ? id : undefined}
    >
      <picture>
        <NextImage field={props.fields.Image} className="" width={1920} height={720} />
      </picture>
      <div className="hero-sc-overlay">
        <div className="container hero-sc-content">
          {(isPageEditing || props.fields?.Title?.value) && (
            <h1 className="hero-sc-title">
              <Text field={props.fields.Title} />
            </h1>
          )}
          {(isPageEditing || props.fields?.Text?.value) && (
            <div className="hero-sc-prompt">
              <RichText field={props.fields.Text} />
            </div>
          )}
          <div className="hero-sc-search-row" role="search">
            <div className="hero-sc-search">
              {isPageEditing ? (
                <div className="hero-sc-search-editing">Search the court website...</div>
              ) : (
                <PreviewSearch
                  rfkId={PREVIEW_WIDGET_ID}
                  isOpen={isSearchOpen}
            ...
            {(isPageEditing || props.fields?.Link?.value?.href) && (
              <Link field={props.fields.Link} className="hero-sc-search-btn" />
            )}
          </div>
        </div>
      </div>
