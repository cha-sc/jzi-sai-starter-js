...
/* SuperiorCourtAnnouncement — skyline band + featured card + View All */
export const SuperiorCourtAnnouncement = (props: PromoCtaProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  const skylineUrl = props.params?.BackgroundImageUrl || '';

  return (
    <div
      className={`component promo-cta promo-cta-sc-announce ${sxaStyles}`}
      id={id ? id : undefined}
      style={skylineUrl ? { backgroundImage: `url("${skylineUrl}")` } : undefined}
    >
      <div className="promo-cta-sc-announce-inner">
        <div className="container">
          ...
          <div className="promo-cta-sc-card">
            {(isPageEditing || props.fields?.Image?.value?.src) && (
              <div className="promo-cta-sc-card-media">
                <NextImage field={props.fields.Image} width={640} height={400} />
              </div>
            )}
            <div className="promo-cta-sc-card-body">
              <h3 className="promo-cta-sc-card-title">
                <Text field={props.fields.Title} />
              </h3>
              {(isPageEditing || props.fields?.Text?.value) && (
                <RichText field={props.fields.Text} className="promo-cta-sc-card-text" />
              )}
              {(isPageEditing || props.fields?.Link?.value?.href) && (
                <Link field={props.fields.Link} className="promo-cta-sc-read-more" />
              )}
            </div>
          </div>
          {(isPageEditing || props.fields?.Link2?.value?.href) && (
            <div className="promo-cta-sc-view-all">
              <Link field={props.fields.Link2} className="promo-cta-sc-view-all-btn" />
            </div>
          )}
        </div>
      </div>
