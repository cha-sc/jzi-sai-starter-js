...
const ArticleListSuperiorCourt = (props: ArticleListComponentProps): JSX.Element => {
  const id = props.params?.RenderingIdentifier;
  const newsItems = getNewsItems(props.fields?.items, parseInt(props.params?.NumberOfItems) || 4);
  const allArticlesPageHref = getAllArticlesPageHref(props.fields?.items);
  const sxaStyles = `${props.params?.styles || ''}`;

  return (
    <div
      className={`component component-spaced article-list article-list-superior-court ${sxaStyles}`}
      id={id ? id : undefined}
    >
      <div className="container">
        <div className="article-list-sc-header">
          <h2 className="sc-section-title">NEWS AND EVENTS</h2>
          <Link href={allArticlesPageHref} className="article-list-sc-view-all">
            VIEW ALL NEWS &amp; EVENTS →
          </Link>
        </div>
        <div className="article-list-sc-grid">
          {newsItems?.map((item) => (
            <article className="article-list-sc-card" key={item.url}>
              <Link href={item.url} className="article-list-sc-thumb">
                <NextImage field={item.fields.Thumbnail} width={600} height={360} />
              </Link>
              <h3 className="article-list-sc-title">
                <Link href={item.url}>
                  <Text field={item.fields.Title} />
                </Link>
              </h3>
              <p className="article-list-sc-excerpt">
                <Text field={item.fields.Excerpt} />
              </p>
            </article>
          ))}
...
export const SuperiorCourt = withDatasourceCheck()<ArticleListComponentProps>(
  ArticleListSuperiorCourt
);
