...
/** PJM hero overlay: semi-transparent panel with topic list + primary CTA */
export const HeroOverlayPanel = (props: PromoCtaProps): JSX.Element => {
  const id = props.params.RenderingIdentifier;
  const { page } = useSitecore();
  const isPageEditing = page.mode.isEditing;
  const sxaStyles = `${props.params?.styles || ''}`;
  ...
  return (
    <aside
      className={`component promo-cta hero-overlay-panel ${sxaStyles}`}
      id={id || undefined}
      aria-label="Trending topics"
    >
      <div className="hero-overlay-panel__inner">
        <div className="hero-overlay-panel__header">
          <Text field={props.fields.Eyebrow} tag="h2" />
        </div>
        <div className="hero-overlay-panel__body">
          <RichText field={props.fields.Text} />
        </div>
        <div className="hero-overlay-panel__cta">
          {(isPageEditing || props.fields?.Link?.value?.href) && (
